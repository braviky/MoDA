...
class DistanceToBins(nn.Module):
    ...
    def __init__(self, dist_min=0.0, dist_max=20.0, num_bins=64, use_onehot=False):
        ...
        if use_onehot:
            ...
            offset = torch.linspace(dist_min, dist_max, self.num_bins-1)                     
            self.coeff = -0.5 / ((offset[1] - offset[0]) * 0.2).item() ** 2                                    
    ...
    def forward(self, dist, dim, normalize=True):
        ...
        if self.use_onehot:
            diff = torch.abs(dist - self.offset.view(*offset_shape))                       
            bin_idx = torch.argmin(diff, dim=dim, keepdim=True)                
            y = torch.zeros_like(diff).scatter_(dim=dim, index=bin_idx, value=1.0)
        else:
            overflow_symb = (dist >= self.dist_max).float()                
            y = dist - self.offset.view(*offset_shape)                         
            y = torch.exp(self.coeff * torch.pow(y, 2))                         
            y = torch.cat([y, overflow_symb], dim=dim)                       
            if normalize:
