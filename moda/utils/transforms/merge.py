...
class MergeChains(object):                                                                                    
    ...
    def _data_attr(self, data, name):
        if name in ('generate_flag', 'anchor_flag', 'fr_flag') and name not in data:
            return torch.zeros(data['aa'].shape, dtype=torch.bool)
        elif name in ('cdr_flag', 'fix_cdr_flag') and name not in data:                            
    ...
    def __call__(self, structure):
        ...
        data_out['coalition_type'] = structure.get('coalition_type', -1)
        return data_out
